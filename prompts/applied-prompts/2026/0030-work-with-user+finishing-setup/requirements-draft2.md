...
1. `${ENTITY_NAME}RequestDto` — fields matching all writable entity fields (exclude `id`, `version`, audit fields).
...
- Exclude `id` and `version` from `toEntity()` mapping using `@Mapping(target = "id", ignore = true)` and `@Mapping(target = "version", ignore = true)`.

---

## STEP 3 — Create Abstract Repository
...
Create an abstract base for repository tests using Testcontainers in `src/test/java` or `src/integration-test/java` (as per project structure).

- Use `@DataJpaTest` and `@AutoConfigureTestDatabase(replace = Replace.NONE)`.
- Use PostgreSQL container.
- Test basic CRUD and custom queries.
...
TODO: Servise Integration AbstractIntegrationTest

## STEP 8 — Service Integration Tests

Create integration tests for `${SERVICE}` using Testcontainers. Verify business logic, validation, and mapping.
...
- Inject `${SERVICE}`.
- Expose REST endpoints: POST `/`, GET `/{id}`, GET `/`, PUT `/{id}`, DELETE `/{id}`.
- TODO: Add support for pagination.
- Use `@Valid` for requests.
- Add OpenAPI documentation with `@Operation`, `@ApiResponse`, etc.

---

TODO: Controller Integration AbstractTest

## STEP 10 — Controller Integration Tests

Create controller integration tests using `MockMvc` and `@SpringBootTest`. Use Testcontainers to ensure full end-to-end flow.
...
## STEP 12 — Optimistic Locking and Global Error Handling

1. Add `@Version` field to `${ENTITY_NAME}` if not present.
2. Include `version` in DTOs.
