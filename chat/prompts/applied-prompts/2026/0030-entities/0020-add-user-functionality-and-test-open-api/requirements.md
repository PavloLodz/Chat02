...
Create an abstract base class `AbstractRepositoryIntegrationTest` for repository tests using Testcontainers in `src/test/java/${REPO_BASE_PACKAGE}`:
...
Create an abstract base class `AbstractControllerIntegrationTest` in `src/test/java/${CONTROLLER_PACKAGE}/`:
