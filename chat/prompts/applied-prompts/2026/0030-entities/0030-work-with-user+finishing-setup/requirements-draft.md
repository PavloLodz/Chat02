...
- Do not place repository injection or business logic in this interface — keep it as a pure contract.
- Throw `EntityNotFoundException` (from `${EXCEPTION_PACKAGE}`) when entity is not found — implement this in `${SERVICE}` in Step 8.
