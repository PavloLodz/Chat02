...
Create `${ENTITY_NAME}RepositoryIT` in `src/integration-test/java/${REPO_PACKAGE}` extending `AbstractRepositoryIT`:
...
Create `${ENTITY_NAME}ServiceIT` in `src/integration-test/java/${SERVICE_PACKAGE}` extending `AbstractServiceIT`:
...
Create `${ENTITY_NAME}ControllerIT` in `src/integration-test/java/${CONTROLLER_PACKAGE}` extending `AbstractControllerIT`:
